...
clone(void (*fcn)(void*), void* arg, void* stack)
{
  ...
  //a bunch of the same stuff from fork
  np->parent = proc;
  np->pgdir = proc->pgdir;
  np->sz = proc->sz;
  *np->tf = *proc->tf;
  ...
  returnPC = stack + PGSIZE - (2 * sizeof(void *)); //leave room for these 2
  *(uint*) returnPC = 0xffffffff;
  givenArg = stack + PGSIZE - sizeof(void *);
  *(uint*) givenArg = (uint) arg;
...
join(void** stack)
{
  ...
  acquire(&ptable.lock);
  for(;;){
    //go through process table looking for zombie children
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
